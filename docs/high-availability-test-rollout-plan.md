...
本轮增量实现：新增 `groups.scheduler_config` 分组级调度配置（默认空 JSON，兼容旧分组）；管理端 DTO、服务层、Ent 仓储和前端分组编辑界面已贯通 `strategy`、首字前切号、粘性模式及重试预算字段。高可用分组的非池账号在首字前收到上游 HTTP 524 时，且 `first_byte_failover=true`，跳过通用长重试，直接进入现有 `UpstreamFailoverError` 切号链路；关闭该开关时保留旧重试路径。首字前传输层错误也转换为可切换错误。公共转发、Bedrock 和 Anthropic API Key 透传路径均复用该请求级判断。`legacy` 与池模式仍保留旧行为。分组策略读取使用 5 秒短 TTL 进程缓存，旧分组未配置时回退旧全局 OpenAI 策略。真实上游故障矩阵和 Docker 数据副本验证仍待后续阶段。

本轮同时将 `max_account_switches` 的分组覆盖接入 OpenAI、Gemini、Grok 媒体和通用网关入口；未配置时继续使用全局默认值。`same_account_retry_attempts` 已接入公共网关、OpenAI、Gemini、Grok 媒体和搜索等失败循环；仅在高可用分组生效，legacy 继续使用账号级 `pool_mode_retry_count`。
...
- 分组调度新增 `selection_mode`：`weighted`（默认，保持当前 Top-K 加权探索）和 `strict_health`（显式开启后按综合健康分确定性选择最高分账号）。旧分组、`legacy` 分组和未配置该字段的高可用分组继续使用 `weighted`，不改变现有流量分布。
- `strict_health` 只改变候选排序，不改变模型/平台/资格/配额/RPM/并发过滤、粘性硬绑定的协议、failover 次数和首字后禁止切号边界；没有有效健康样本时仍按原有探索和稳定排序兜底。
- OpenAI 同步成功请求新增低权重总耗时样本。总耗时按输出 token 数做温和归一化（避免长输出天然被判慢），只参与健康评分，不写入首字时间、计费记录或数据库。
- 同步总耗时样本使用独立 EWMA、TTL 和低权重；失败请求仍按错误分类处理，不因请求级错误惩罚账号。流式请求继续以首字时间为主，避免把用户主动长时间消费流的总时长误判为上游慢。
- 观测日志新增候选的 `total_latency_ms`、`total_latency_sample_source` 和最终 `selection_mode`，便于验证严格模式是否生效。
...
- **高可用分组 + 非池模式 + 上游 HTTP 524 + 首字前**是强制快速切号场景：不执行当前账号的长时间通用重试，收到 524 后立即生成 `UpstreamFailoverError`，排除当前账号并选择同分组的下一个候选账号。只要客户端连接仍有效且尚未向下游写出有效内容，切号可以在同一次请求内完成。
- 高可用分组中的非池账号，对其他可判定为账号级故障的首字前错误也应优先跨账号切换；同账号退避重试必须受分组策略和总耗时预算限制，不能阻塞候选账号切换。
- `legacy` 分组保持现有通用重试和池模式语义，不因本策略升级而改变行为。
- 池模式的同账号重试保持原有 `pool_mode_retry_count` 与状态码配置；即使收到 524，也只有在该账号的池模式重试规则明确允许时才先原地重试。
- 首字后流错误继续禁止换号，避免拼接两个账号的响应。
- 客户端已断开、请求上下文已取消或候选账号耗尽时，不再继续切号，按最终错误返回。
- 上游明确返回的 HTTP 524 与传输层 `context deadline exceeded`/连接重置分开统计；前者在高可用非池模式下必须立即切号，后者也应在首字前转换为可切换的账号级失败，不能直接降级为不可切换的 502。
...
3. 模拟上游：高可用非池模式下的 524 立即切号（验证不发生长时间同账号重试）、429、500、503、慢首字、连接超时、首字后断流和确定性 400；同时验证 legacy 分组仍保持旧重试行为、池模式仍遵守原有同账号重试配置。
...
验证状态：已使用 Go 1.26.5 完成本轮改动文件格式化；目标编译检查 `./internal/service ./internal/handler ./internal/repository ./internal/handler/dto ./internal/handler/admin` 通过，迁移包和迁移相关仓储测试通过；新增的高可用权重、跨平台健康排序、健康样本 TTL、legacy 排序隔离、健康逃逸粘性模式、failover 绑定、错误分类、健康熔断、冷启动配额、定时探测成功率与延迟低权重融合和观测指标/日志测试通过。Compose 配置静态展开检查通过，并确认测试端口默认绑定 `0.0.0.0:18080`、数据库/Redis 使用独立 `ha-test` 数据目录；当前 Docker Desktop 未启动，无法执行镜像构建和副本启动。前端依赖安装因 npm registry 网络超时只生成了不完整的 `node_modules/.pnpm` 缓存，缺少 `vue-tsc` 可执行文件，类型检查/构建未执行；全量服务测试、真实上游故障矩阵和上线演练仍待统一验证阶段执行。
...
### 9.4 下一版实现进度（严格健康优先与同步总耗时）

已完成本地代码实现：

- 分组新增 `scheduler.selection_mode`，空值兼容为 `weighted`；新增 `strict_health` 严格健康优先模式。
- 严格模式在资格过滤、并发、配额和 failover 不变的前提下，按综合健康分确定性选择最高分账号；加权模式保持原有探索行为。
- 成功非流式请求记录总耗时，并按 `total_latency_ms / sqrt(max(output_tokens, 1))` 做温和归一化，以低权重参与高可用评分；流式完整消费时长不计入该信号。
- Responses、Chat Completions 和 Messages 成功路径已接入 ForwardResult 指标上报；旧上报接口保持兼容。
- 候选调度日志增加 `selection_mode`、`total_latency_ms` 和样本存在标记。
- 前端分组创建/编辑页面已增加选择模式控件；旧分组和未配置字段可继续使用。
- 已补充分组配置、严格排序、总耗时归一化测试；Go 配置包和相关服务定向测试通过。

前端已完成静态检查：类型字段、模板绑定、创建/编辑初始化、重置、回填和请求提交链路一致。按当前要求不执行依赖安装、类型构建或完整前端构建。

更新分组后使用接口返回的最新对象立即回写列表，避免编辑弹窗在异步刷新完成前读取旧的选择模式。

待完成：完整后端回归测试、Docker 数据副本验证，以及测试通过后的提交和推送。`docker_log.txt` 为用户本地观测文件，保持未跟踪，不纳入提交。
