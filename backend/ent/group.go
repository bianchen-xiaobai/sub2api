...
type Group struct {
	...
	// 分组级高可用调度配置；空值保持 legacy 行为
	SchedulerConfig map[string]interface{} `json:"scheduler_config,omitempty"`
	// 是否启用模型路由配置
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSchedulerConfig, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldSchedulerConfig:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field scheduler_config", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.SchedulerConfig); err != nil {
					return fmt.Errorf("unmarshal field scheduler_config: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("scheduler_config=")
	builder.WriteString(fmt.Sprintf("%v", _m.SchedulerConfig))
	builder.WriteString(", ")
